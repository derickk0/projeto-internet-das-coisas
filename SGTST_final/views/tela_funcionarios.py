...
def abrir_tela_funcionarios():
    ...
    campos = [
        ("ID", 0), ("Nome", 1), ("Cargo", 2), ("Departamento", 3), ("Salário", 4),
        ("Telefone", 5), ("Celular", 6), ("Email", 7),
        ("Logradouro", 8), ("Número", 9), ("Bairro", 10), ("Cidade", 11), ("UF", 12), ("CEP", 13)
    ]
    entradas = []
    ...
    def salvar():
        ...
        id_func = valores[0]
        ids_existentes = [f['id'] for f in funcionarios.listar_funcionarios_dados()]
    ...
    ttk.Button(frame_cadastro, text="Salvar", command=salvar).grid(row=14, column=0, columnspan=2, sticky="ew", pady=5, padx=20)
    ttk.Button(frame_cadastro, text="Ir para Lista de Funcionários", command=mostrar_lista).grid(row=15, column=0, columnspan=2, sticky="ew", pady=5, padx=20)
    ...
    def atualizar_lista():
        ...
        for i, func in enumerate(dados):
            lista.insert(tk.END, f"ID: {func.get('id', '')} | Nome: {func.get('nome', '')} | Cargo: {func.get('cargo', '')} | Departamento: {func.get('departamento', '')} | Salário: {func.get('salario', '')}")
            ...
            if i < len(linhas):
                ...
                if len(partes) >= 14:
                    endereco = f"Logradouro: {partes[8]} | Número: {partes[9]} | Bairro: {partes[10]} | Cidade: {partes[11]} | UF: {partes[12]} | CEP: {partes[13]}"
    ...
    def excluir_funcionario():
        ...
        if selecionado:
            ...
            if funcionario_idx < len(dados):
                funcionarios.excluir_funcionario(dados[funcionario_idx]['id'])
                atualizar_lista()
