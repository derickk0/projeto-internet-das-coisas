...
def abrir_tela_sensores():
    ...
    def ler_serial():
        if not arduino or not arduino.is_open:
            labels["Presença"].set("Sem conexão")
            labels["Luminosidade"].set("Sem conexão")
            labels["Temperatura"].set("Sem conexão")
            return
        try:
            linha = arduino.readline().decode(errors='ignore').strip()
            # Exemplo: Presenca: 1 | Luminosidade: 800 | Temperatura: 32.5C 90.5F
