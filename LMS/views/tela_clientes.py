...
def abrir_tela_clientes():
    ...
    campos = [
        ("CNPJ/CPF", 0), ("Nome", 1), ("Observações", 2), ("Telefone", 3), ("Celular", 4), ("Email", 5),
    ...
    def salvar():
        ...
        cnpjCpf = valores[0]
        ids_existentes = [c['cnpjCpf'] for c in clientes.listar_clientes_dados()]
        if cnpjCpf in ids_existentes:
            messagebox.showerror("Erro", f"Já existe um cliente com o CNPJ/CPF {cnpjCpf}.")
            return
    ...
    def atualizar_lista():
        ...
        for i, cli in enumerate(dados):
            lista.insert(tk.END, f"CPF/CNPJ: {cli.get('cnpjCpf', '')} | Nome: {cli.get('nome', '')} | Telefone: {cli.get('telefone', '')} | Celular: {cli.get('celular', '')} | Email: {cli.get('email', '')}")
            lista.insert(tk.END, f"Observações: {cli.get('observacoes', '')}")
            if i < len(linhas):
    ...
    def excluir_cliente():
        ...
        if selecionado:
            ...
            if cliente_idx < len(dados):
                clientes.excluir_cliente(dados[cliente_idx]['cnpjCpf'])
                atualizar_lista()
