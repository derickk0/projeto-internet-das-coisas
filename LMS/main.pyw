...
def main():
    ...
    label_titulo = ttk.Label(frame_principal, text="LAST MILE SAFETY", font=("Segoe UI", 32, "bold"))
    label_subtitulo = ttk.Label(frame_principal, text="Sistema de Gestão de Transporte e Segurança", font=("Segoe UI", 21))
    label_titulo.grid(row=0, column=0, pady=(0, 40))
    label_subtitulo.grid(row=0, column=0, pady=(100, 40))
    ...
    def mostrar_menu(menu):
        ...
        ttk.Label(frame_menu, text=menu, font=("Segoe UI", 50, "bold")).grid(row=0, column=0, pady=(0, 30))
        row = 1
        def voltar():
            frame_menu.destroy()
    ...
    ttk.Button(frame_principal, text="ADM", command=lambda: mostrar_menu("ADM"), width=20, style="Main.TButton").grid(row=3, column=0, pady=10, ipady=10, sticky="nsew")
